  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
        ...
        <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
          ...
          <img onClick={()=> setSelectedUser(null)} src={assets.arrow_icon} alt="close chat" 
          className='md:hidden max-w-7' />
          <img src={assets.help_icon} alt="test" className='max-md:hidden max-w-5' />
        </div>
              ...
              !== authUser._id && 'flex-row-reverse'}`}>
                {msg.image ? (
                  ...
                  <p className={`p-2 max-w-[200px] md:text-sm font-light 
                     mb-8 break-all bg-gray-500/30 text-white ${msg.senderId
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
            <div className='flex-1 flex items-center bg-gray-100/12 px-3'>
              ...
              onKeyDown={(e)=> e.key === "Enter" ? handleSendMessage(e) : null } type="text"
               placeholder="Send" className='flex-1 text-sm p-3 border-none 
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
      ...
      <p className='text-lg font-medium text-white'>Mono Chat, this is odd room chat.</p>
    </div>
  )
}
